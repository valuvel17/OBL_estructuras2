...
private:
    ...
    libroAVL *addAux(libroAVL *nodo, int id, string titulo)
    {
        ...
        if (id > nodo->id)
            nodo->der = addAux(nodo->der, id, titulo);
        if (id < nodo->id)
            nodo->izq = addAux(nodo->izq, id, titulo);
        if (nodo->id == id)
        {
            nodo->titulo = titulo;
            if (!(nodo->estado))
        ...
        { // estoy preguntando si el dato es mayor en este caso seria escenario derecha derecha
    ...
    string findAux(libroAVL *nodo, int id)
    {
        ...
        if (nodo->id > id)
            return findAux(nodo->izq, id);

        if (nodo->id < id)
            return findAux(nodo->der, id);

        if (nodo->estado)
            return nodo->titulo + " H";
        else
            return nodo->titulo + " D";
    }
    ...
    bool toggleAux(libroAVL *&nodo, int id)
    {
        ...
        if (nodo->id > id)
            return toggleAux(nodo->izq, id);

        if (nodo->id < id)
            return toggleAux(nodo->der, id);
    ...
    void destruirAVL(libroAVL *nodo)
    {
        if (!nodo)
            return;
        if (nodo->izq)
            destruirAVL(nodo->izq);
        if (nodo->der)
            destruirAVL(nodo->der);
